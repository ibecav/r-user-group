...
R/RStudio, pak, and the Tidyverse.

Note that this document may not suffice as a **"DIY"** resource. This document 
is meant as a short tutorial outline and notes.  You're welcome to contact the 
author for questions and assistance
...
(Tired of typing? See @tip-copypaste you can just copy and paste)
...
similar to this... (See @tip-gitgithub for what version control is for).

![Screenshot showing project wizard](hints_images/project_wizard_types.png)

So many options!  Sorry, not sorry, as you grow accustomed to RStudio
you'll grow to love the many options.  But for now choose `New Project`.

![Screenshot showing project wizard](hints_images/project_details.png)

Give your new project a meaningful name, even if it is a bit silly like this 
example. If you look using your operating system it really does
...
and label it.  You can use the browse button to do that now. Here's what mine 
looks like on a windows machine 

![Project meta directory](hints_images/example_proj_directory.png)

when you press the `Create Project` button there will be a very short
pause and you'll have a new project.

Follow this process **every time** you start a new work flow.

Want to switch between projects?  That's what `Open Project...` and
`Recent Projects` are for.

What's in a project?  It's actually a directory on your file system.
It should contain everything you need for your projects, the data, the scripts,
any saved objects, a history of your work, etc.. On Windows it looks like ...

![Project directory on Windows](hints_images/sample_proj_directory.png)


### Shortcuts can be useful

One of the advantages of using "projects" in RStudio is the ability it
gives you to create shortcuts to get to the exact project you need.

For example, in my case, I like having choices.  So on a windows machine
I create a folder for all my projects, I have a short cut to start `RStudio`
without specifying any particular project (more on that in a bit), and you
can create one or more shortcuts to specific projects so that it looks like
this screen shot... From left to right there's the folder that encapsulates
all my projects, the desktop shortcut to launch RStudio and a shortcut
to launch the example project in this documentation.

![Shortcuts on Windows desktop](hints_images/Shortcuts_windows.png)

Totally optional but in my case a real time saver.

## Customize your experience

As is the case with most modern software `RStudio` has a plethora of user
options that you can customize to your heart's content. The default settings 
are quite rational but we're going to make a few key tweaks.

There are two levels of options in `RStudio`, `Global` and `Project` as
the names imply you can have options that only apply to specific projects,
but let's focus on options that apply globally.  Please select
`Tools -> Options`.

![Screenshot showing Tools -> Options](hints_images/global_options.png)

The single most important option to set is right there on the very
first screen you see, but it is a bit perplexing to new users so some
explanation is warranted.

![Screenshot showing General Basic Options](hints_images/basic1.png)


::: {.callout-important}
You may be tempted to think you want to "always" save your workspace on exit. 
I mean who doesn't want to save their work right? The problem is that doesn't
mean what you think it means (*Princess Bride* reference).  The problem
is you will be saving some `R` objects but not leaving yourself in a 
reproducible state.  And if the things you are save are large, it will feel 
sooooooo slow. I **strongly** encourage you to select "never"!

While you're at it make sure you are saving your "History" as shown.

And if you followed my advice to keep all your projects in one place
might as well handle that too.
:::
...
::: {#tip-gitgithub .callout-tip}
### If you work in a team

Then learn how to use `git` and `github` soon.  If you have 
more than 2 people in the team you'll very quickly learn
that emailling around (or dropping into `Teams`) is a recipe for disaster!

:::


::: {#tip-short_cuts .callout-tip}
### Shortcuts can be useful
